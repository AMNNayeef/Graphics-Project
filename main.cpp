...
void updateBird(int value)           // Where and how to move
{
    if(positionBird >4.0)
        positionBird = -1.5f;
    positionBird += speedBird;
	glutPostRedisplay();
	glutTimerFunc(1, updateBird, 0);
}

void soundMorning()
{

    PlaySound("Morning-sounds.wav", NULL,SND_ASYNC|SND_FILENAME|SND_LOOP);
...
void soundNight()
{
    ...
    PlaySound("Night-sounds.wav", NULL,SND_ASYNC|SND_FILENAME|SND_LOOP);
...
///Birds
glPushMatrix();
glTranslatef(positionBird,positionBird, 0.0f);
glLineWidth(3.0);
glBegin(GL_LINES);
{
  glColor3f(0.0f,0.0f,0.0f);
